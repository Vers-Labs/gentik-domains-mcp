...
const mockResponses = {};
function mockApi(urlPattern, status, body) {
...
describe("MCP Tools", () => {
    ...
    it("register_domain: registers with invoice payment", async () => {
        ...
        const registerCall = calls.find((c) => c[0].includes("/api/domains/register"));
    ...
    it("add_dns_record: adds A record", async () => {
        ...
        const calls = vi.mocked(globalThis.fetch).mock.calls;
        const dnsCall = calls.find((c) => c[0].includes("/dns"));
    ...
    it("add_dns_record: includes optional MX fields", async () => {
        ...
        const calls = vi.mocked(globalThis.fetch).mock.calls;
        const dnsCall = calls.find((c) => c[0].includes("/dns"));
        const body = JSON.parse(dnsCall[1]?.body);
    ...
    it("list_tlds: returns TLD list without auth", async () => {
        ...
        const calls = vi.mocked(globalThis.fetch).mock.calls;
        const tldsCall = calls.find((c) => c[0].includes("/api/domains/tlds"));
        const headers = tldsCall[1]?.headers;
