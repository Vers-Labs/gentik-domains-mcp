...
describe("GentikClient", () => {
    ...
    it("uses GENTIK_API_URL from environment", async () => {
        ...
        expect(fetchSpy).toHaveBeenCalledWith("https://test.example.com/api/test", expect.any(Object));
    });
    it("defaults to agentdomains.dev when no URL set", async () => {
        ...
        const client = new GentikClient();
        await client.get("/api/test");
        expect(fetchSpy).toHaveBeenCalledWith("https://agentdomains.dev/api/test", expect.any(Object));
    });
    ...
    it("throws when auth required but no API key set", async () => {
        ...
        const client = new GentikClient();
        await expect(client.get("/api/account", true)).rejects.toThrow("GENTIK_API_KEY is required");
    });
    ...
    it("parses JSON responses", async () => {
        vi.spyOn(globalThis, "fetch").mockResolvedValue(new Response(JSON.stringify({ domain: "test.com", available: true }), {
            status: 200,
        }));
        const client = new GentikClient();
    ...
    it("throws on API error with error+hint fields", async () => {
        ...
        const client = new GentikClient();
        await expect(client.get("/api/test")).rejects.toThrow("Rate limit exceeded (hint: Try again in 60 seconds)");
    });
    it("throws on API error with details", async () => {
        ...
        const client = new GentikClient();
        await expect(client.get("/api/test")).rejects.toThrow('Validation failed\nDetails: {"domain":"required"}');
    });
